...
| External orchestration framework adapter | An adapter that sends work to an external swarm or harness framework | Potentially richer orchestration models, stronger role lifecycle, more backend features | Highest integration cost, highest mismatch risk with current skills, more state and boundary complexity, weakest guarantee of preserving existing local behavior | Only after lighter options prove insufficient | **Third choice** |
...
- it leaves room to add tmux-based or external-framework backends later without rewriting `tgtool`
...
3. external orchestration framework adapter
