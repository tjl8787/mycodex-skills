...
- `multi-codex-orchestration`: Use when the user explicitly wants multiple Codex agents or a virtual team, the work can be decomposed into safe parallel roles, and tgtool should coordinate staged orchestration. It includes a runnable native wrapper path, automated `ruflo` backend bootstrap helpers for Codex/runtime/swarm setup, a visible `tmux` pane bootstrap for foreground worker sessions, and now defaults that visible backend to shell-first panes so they can execute real shell commands or one-shot `codex` tasks deterministically while still supporting an explicit interactive `codex` pane mode when needed. It enables mouse and clipboard support, writes a session state file, supports live role dispatch from the main session into visible panes, supports stage-wide broadcast updates, and includes helper scripts for pane-side shell execution and one-shot Codex task execution, plus Phase 2 claude-flow adapter docs and payload/normalization scripts.
