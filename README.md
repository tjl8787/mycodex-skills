...
- `multi-codex-orchestration`: Use when the user explicitly wants multiple Codex agents or a virtual team, the work can be decomposed into safe parallel roles, and tgtool should coordinate staged orchestration. Phase 1 includes a runnable native wrapper path plus a `ruflo` init helper for Codex/runtime backend setup, and Phase 2 adds claude-flow adapter docs plus payload and normalization scripts.
