...
- `multi-codex-orchestration`: Use when the user explicitly wants multiple Codex agents or a virtual team, the work can be decomposed into safe parallel roles, and tgtool should coordinate a Phase 1 codex-native orchestration path. Phase 1 now includes a wrapper spec, session schema, execution playbook, bootstrap script, role-brief renderer, and starter session template.
