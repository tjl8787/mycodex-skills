...
- `multi-codex-orchestration`: Use when the user explicitly wants multiple Codex agents or a virtual team, the work can be decomposed into safe parallel roles, and tgtool should coordinate staged orchestration. It includes a runnable native wrapper path, automated `ruflo` backend bootstrap helpers for Codex/runtime/swarm setup, a visible `tmux` pane bootstrap for foreground worker sessions that auto-launches `codex` in each role pane by default, enables mouse and clipboard support, writes a session state file, supports live role dispatch from the main session into visible panes, and supports stage-wide broadcast updates, plus Phase 2 claude-flow adapter docs and payload/normalization scripts.
