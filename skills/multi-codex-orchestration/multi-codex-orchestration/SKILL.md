...
### 1.5. Token mode policy

Always use low-token mode for multi-agent orchestration:
- Use `scripts/render_role_brief.py` low-token strategy.
- Truncate `task_summary` and `requested_outcome`.
- Emit concise `constraints` and `support_layers`.
- Avoid sending full context every time.

Delta updates are still allowed, but only as incremental updates on top of low-token mode:
- `python3 scripts/render_role_brief.py session.json --role implementer --delta --new-instructions "..."`

Then continue with planning/dispatch.
